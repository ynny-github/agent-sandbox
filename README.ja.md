...
その子コマンドまで生き残るかどうかで決まります。各ホップは呼び出し元から
...
は文字通り無制限であり、「上限まで」ではありません。このリポジトリ自身の
3 ホップチェーン (`agent-sandbox` → `git` → `realgit`) を模した構成で、
上限を `network_profile: "developer"` に固定して実測したところ:
`HTTP_PROXY`/`HTTPS_PROXY` をすべてのホップの `allow_vars` で通した場合、
`realgit` は developer の許可リスト外のドメインに対して本物のプロキシ拒否
(`CONNECT tunnel failed, response 403`) を受け取りました。1 か所のホップの
`allow_vars` だけを元の狭いリストに戻すと、同じリクエストは実際のホストに
届いてしまいました。**このリポジトリ自身のプロファイルは、`git` チェーンの
すべてのホップ (セッションのトップレベルを含む) で同一の狭い
`environment.allow_vars` — `["PATH", "HOME", "USER", "LANG", "TERM"]`、
プロキシ変数なし — を設定しています。** そのため `realgit` の `allow_all`
はここでは上限に絞られません。`network_profile: "developer"` と
`network_profile: null` のどちらでも同じ結果でした: 上限をどちらに
選んでも、`git` が到達できる範囲は変わりません。
...
を持っています。このリポジトリのセッション上限 (トップレベルの `network`
セクション) は `{"network_profile": null}` — 無制限 — ですが、git の
到達範囲が絞られていないのはこの設定のせいではありません: このチェーン
(`agent-sandbox` → `git` → `realgit`、セッション自身のトップレベルを含む)
のすべてのホップが `environment.allow_vars` を
`["PATH", "HOME", "USER", "LANG", "TERM"]` に制限しており、上限を
どちらに設定していても nono のプロキシ用環境変数が `realgit` まで
届くことはありません — そのため `realgit` はプロキシを介さず直接接続します
(実測の詳細は上の「ネットワーク」を参照)。git がネットワーク越しに到達
できる範囲はまったく絞られていません: ホストがルーティングできる宛先
ならどこへでも届き、このラッパーがチェックする何かがそれを狭めることは
ありません。

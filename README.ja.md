...
ものでもありません。** 上記の許可リストが絶対的なのは「そもそも実行され
得るプログラムはどれか」についてであり — これは何によっても回避されません。
しかし、それ自身がコンパイラやインタプリタであるコマンドが、渡されたコード
で何をできるかについては何も述べていません: そのコマンドを縛るのは *その
コマンド自身の* サンドボックスが到達できる範囲だけであり、argv ルールでも、
プロファイルの他の箇所でどのツールが列挙されているかでもありません。この
リポジトリ自身のプロファイルが `go` をまさにこの理由で列挙しています —
それが実際どれだけの代償を伴い、`go` 自身の許可の内側から Go プログラムが
何をできるかを詳しく見たときに封じ込めがどこまで届くのかは、下の
...
- コマンドプロファイルが存在し、`nono profile validate` を通り、自身の
  ...
  `fs_write` も確認します (このプロファイルのレビューで見つかった 2 つの
  Critical な指摘は、どちらもトップレベルのリストではなくコマンド自身の
  `fs_write` に潜んでいました)。`$WORKDIR` だけを通じて表現された許可は
  それでもこのチェックからは見えません — 起動時の最終判断は常に nono 自身が
  下します。
...
- **コードをコンパイルして実行するツールチェーンは、他にどのコマンドが
  ...
  ではありません。動的リンクされたバイナリを (フローとも `go` とも読み取り

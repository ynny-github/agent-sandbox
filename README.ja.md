...
自分で書く前に知っておくべき性質が 3 つあります:

- **ブローカー自身のエントリは、自分のディレクトリを `executable_dirs` に
  列挙しなければなりません。** ランチャーは `agent-sandbox broker` を絶対パスでは
  なく必ずベース名で起動します: nono は宣言済みポリシーコマンドへの絶対パス
  呼び出しを direct exec bypass として拒否するため、ブローカー自身のエントリ
  ポイントの解決は、ランチャー自身の `PATH` ではなく
  `command_policies.executable_dirs` と固定された `executable` を経由します。
  `agent-sandbox` をポリシーコマンドとして宣言するすべてのプロファイル —
  上の例と同じ形のものはすべて該当します — は、インストール済みバイナリの
  ディレクトリを `executable_dirs` に含める必要があります。含めなければ
  セッション全体が起動に失敗します。`agent-sandbox doctor` はまさにこれを
  確認します。

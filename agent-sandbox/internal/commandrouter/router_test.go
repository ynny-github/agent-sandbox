...
func TestRoute_MatchingPattern_ReturnsHost(t *testing.T) {
	if got, _ := commandrouter.Route("git status", []string{"git *"}, nil); got != "host" {
		t.Errorf("got %q, want host", got)
	}
}

func TestRoute_NoMatchingPattern_ReturnsContainer(t *testing.T) {
	if got, _ := commandrouter.Route("npm test", []string{"git *"}, nil); got != "container" {
		t.Errorf("got %q, want container", got)
	}
}

func TestRoute_EmptyPatterns_ReturnsContainer(t *testing.T) {
	if got, _ := commandrouter.Route("git status", nil, nil); got != "container" {
		t.Errorf("got %q, want container", got)
	}
	if got, _ := commandrouter.Route("git status", []string{}, nil); got != "container" {
		t.Errorf("got %q, want container", got)
	}
}
...
func TestRoute_MultiplePatterns_MatchesSecond(t *testing.T) {
	...
	if got, _ := commandrouter.Route("make build", patterns, nil); got != "host" {
		t.Errorf("got %q, want host", got)
	}
	if got, _ := commandrouter.Route("npm run test", patterns, nil); got != "host" {
		t.Errorf("got %q, want host", got)
	}
}

func TestRoute_RawStringMatching_NoShellExpansion(t *testing.T) {
	if got, _ := commandrouter.Route("echo $HOME", []string{"git *"}, nil); got != "container" {
		t.Errorf("got %q, want container", got)
	}
	if got, _ := commandrouter.Route("echo $HOME", []string{"echo $HOME"}, nil); got != "host" {
		t.Errorf("got %q, want host", got)
	}
}

func TestRoute_WildcardMatchesSlash(t *testing.T) {
	if got, _ := commandrouter.Route("git add src/main.go", []string{"git *"}, nil); got != "host" {
		t.Errorf("got %q, want host", got)
	}
	if got, _ := commandrouter.Route("go test ./...", []string{"go test *"}, nil); got != "host" {
		t.Errorf("got %q, want host", got)
	}
}
...
func TestRoute_RegexMetacharactersMatchLiterally(t *testing.T) {
	...
	if got, _ := commandrouter.Route("echo /tmp/file.txt", []string{"echo /tmp/file.txt"}, nil); got != "host" {
		t.Errorf("got %q, want host", got)
	}
	if got, _ := commandrouter.Route("echo /tmp/fileXtxt", []string{"echo /tmp/file.txt"}, nil); got != "container" {
		t.Errorf("got %q, want container", got)
	}
}

func TestRoute_ExactMatch_ReturnsHost(t *testing.T) {
	if got, _ := commandrouter.Route("git status", []string{"git status"}, nil); got != "host" {
		t.Errorf("got %q, want host", got)
	}
}
...
func TestRoute_NoPatterns_NilAndEmpty_BothContainer(t *testing.T) {
	...
	for _, patterns := range cases {
		if got, _ := commandrouter.Route("anything", patterns, nil); got != "container" {
...
func TestRoute_MultilineCommitMessage_MatchesHost(t *testing.T) {
	...
	if got, _ := commandrouter.Route(cmd, []string{"git *"}, nil); got != "host" {
		t.Errorf("got %q, want host", got)
	}
}

func TestRoute_DropMatch_ReturnsDrop(t *testing.T) {
	got, matched := commandrouter.Route("rm -rf /tmp/x", nil, []string{"rm -rf *"})
	if got != "drop" {
...
func TestRoute_DropOverridesAllow(t *testing.T) {
	...
	got, matched := commandrouter.Route("git push origin main", allow, drop)
	if got != "drop" {
		t.Errorf("decision = %q, want drop", got)
	}
	if matched != "git push *" {
...
func TestRoute_EmptyDropPatterns_BehaviorUnchanged(t *testing.T) {
	...
	for _, drop := range cases {
		if got, _ := commandrouter.Route("git status", []string{"git *"}, drop); got != "host" {
			t.Errorf("drop=%v: got %q, want host", drop, got)
		}
		if got, _ := commandrouter.Route("npm test", []string{"git *"}, drop); got != "container" {
...
func TestRoute_DefaultContainer_MatchedEmpty(t *testing.T) {
	_, matched := commandrouter.Route("nothing matches", []string{"git *"}, []string{"rm *"})
	if matched != "" {
...
func TestRoute_AllowMatch_ReturnsAllowPattern(t *testing.T) {
	_, matched := commandrouter.Route("git status", []string{"git *"}, nil)
	if matched != "git *" {

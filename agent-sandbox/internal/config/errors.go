...
// gone, so the hook is the only way commands reach execd and there is
...
// configure — the one each command run through execd ran in — no longer
// exists as something agent-sandbox generates; it is the operator-written
// command profile instead.
...
// ErrCommandProfileMissing fires when the nono profile execd runs under is

...
func TestRun_MixedPipe_HostToSandbox(t *testing.T) {
	// `echo hi` on host (allowed) | `cat` in sandbox.
	r := &echoRunner{}
	var out, errb bytes.Buffer
	code, err := router.Run(context.Background(), router.Request{
		Command:       "echo hi | cat",
		AllowPatterns: []string{"echo *"}, // echo→host, cat→sandbox
		CommandRunner: r,
	...
	// Sandbox segment ran as argv (cat), not bash -c the whole pipeline.
	if len(r.gotArgv) != 1 || r.gotArgv[0][0] == "bash" {
		t.Fatalf("sandbox argv = %#v, want a single non-bash cat call", r.gotArgv)
	}
}
...
// mixed-pipeline deadlock: if the downstream sandbox segment exits before
...
func TestRun_MixedPipe_EarlyExitDeadlockGuard(t *testing.T) {
	// `seq 1 1000000` on host produces a large stream; the sandbox segment
	...
	go func() {
		...
		runCode, runErr = router.Run(context.Background(), router.Request{
			...
			AllowPatterns: []string{"seq *"}, // seq→host, cat→sandbox
...
// TestRun_MixedPipe_ThreeSegments verifies that a 3-segment host|sandbox|host
// pipeline correctly wires all inter-segment pipes, closing both ends on finish.
// Routing: "echo *" and "tr *" → host; middle segment → sandbox (echoRunner).
func TestRun_MixedPipe_ThreeSegments(t *testing.T) {
	// echo hello | <sandbox: copy stdin→stdout> | tr a-z A-Z
	// Expected: "HELLO\n"
	r := &echoRunner{}
	var out, errb bytes.Buffer
	code, err := router.Run(context.Background(), router.Request{
		Command:       "echo hello | cat | tr a-z A-Z",
		AllowPatterns: []string{"echo *", "tr *"}, // echo+tr→host, cat→sandbox
		CommandRunner: r,
		Stdout:        &out,
		Stderr:        &errb,
	})
